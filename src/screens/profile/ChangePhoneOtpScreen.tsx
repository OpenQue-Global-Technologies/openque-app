...
type Props = NativeStackScreenProps<ProfileStackParamList, 'ChangePhoneOtp'>;

const RESEND_TIMER_SECONDS = 30;

/**
 * Mocked OTP re-verify for a phone number change (Section 6.2 "Edit
 * Profile"). Reuses Phase 1's OtpInput component and validation logic
 * (state/otp.ts) rather than rebuilding OTP entry from scratch.
 */
export default function ChangePhoneOtpScreen({ navigation, route }: Props) {
  ...
  const handleVerify = async () => {
    if (isOtpCorrect(code)) {
      await updateUserProfile({ phoneNumber: newPhoneNumber });
      navigation.pop(2);
    } else {
      setHasError(true);
    }
  ...
  return (
    <ScreenContainer>
      ...
      <View style={styles.content}>
        ...
        <Body style={styles.body}>Enter the 6-digit code sent to +91-{newPhoneNumber}.</Body>
